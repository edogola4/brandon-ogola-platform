...
export default function ContactPage() {
  ...
  function handleChange<K extends keyof ContactFormData>(key: K, value: ContactFormData[K]) {
    ...
    // run field-level validation using the single-field schema derived from contactSchema
    const shape = contactSchema.shape as Record<string, z.ZodTypeAny>
    try {
      const fieldSchema = shape[key as string]
      if (fieldSchema) {
        const singleSchema = z.object({ [key as string]: fieldSchema })
        const result = singleSchema.safeParse({ [key]: value })
        if (!result.success) {
          const flat = result.error.flatten().fieldErrors as Record<string, string[]>
          setErrors((e) => ({ ...e, [key]: flat[key as string] ?? [] }))
        } else {
          setErrors((e) => ({ ...e, [key]: [] }))
        }
      }
    } catch {
      // ignore field-level validation errors
    }
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.status === 201) {
        ...
        // unexpected but API guaranteed to not 500
        const json = await res.json()
        if (json?.error) {
